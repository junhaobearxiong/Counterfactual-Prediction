...
class EM:
    def __init__(self, y, X, c, J, K, train_pct):
        ...
        self.A = np.full((self.num_patients, self.J, self.N), -.1) + np.random.randn(self.num_patients, self.J, self.N)*0.01 # coefficients a_j's
        ...
        for n in range(self.num_patients):
            ...
            self.coeff_mtx.append(coeff_mtx)
            C = np.stack([self.c[n, :] for i in range(self.last_train_obs[n])], axis = 0)
            C = np.delete(C, nans, axis = 0)
            C_list.append(C)
        self.C_mtx = np.concatenate(C_list, axis = 0)
    ...
    def added_effect(self, n, t):
        ...
        for j in range(self.J):
            if t-1 >= j:
                treatment_effect = treatment_effect + np.dot(self.A[n, j, :], self.X[n, t-1-j, :])
    ...
    def A_mle(self):
        for n in range(self.num_patients):
            rhs = np.subtract(self.y[n, :self.last_train_obs[n]], self.mu_smooth[n, :self.last_train_obs[n]] + np.dot(self.b, self.c[n, :]))
            rhs = np.delete(rhs, np.where(np.isnan(rhs))[0])
            params = sp.linalg.lstsq(self.coeff_mtx[n], rhs)[0] # params as a long vector
            self.A[n, :, :] = np.reshape(params, (self.J, self.N))
    
    def b_mle(self):
        rhs_list = []
        for n in range(self.num_patients):
            treatment_effects = np.zeros(self.last_train_obs[n])
            for t in range(self.last_train_obs[n]):
                effect = 0
                for j in range(self.J):
                    if t-1 >= j:
                        effect += np.dot(self.A[n, j, :], self.X[n, t-1-j, :])
                treatment_effects[t] = effect
            rhs = np.subtract(self.y[n, :self.last_train_obs[n]], self.mu_smooth[n, :self.last_train_obs[n]] + treatment_effects)
            rhs = np.delete(rhs, np.where(np.isnan(rhs))[0])
            rhs_list.append(rhs)
        rhs_concat = np.concatenate(rhs_list, axis = 0)
        params = sp.linalg.lstsq(self.C_mtx, rhs_concat)[0] # params as a long vector
    ...
    def emission(self, z, n, t):
        ...
        for j in range(self.J):
            if t-1 >= j:
                treatment_effect += np.dot(self.A[n, j, :], self.X[n, t-1-j, :])
        pi = treatment_effect + np.dot(self.b, self.c[n, :]) # total added effect
        mean = z + pi
